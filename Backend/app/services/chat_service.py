...
class ChatService:
    ...
    def process_chat(request: ChatRequest, user_id: Optional[Any]) -> ChatResponse:
        ...
        1. Resolve user details and session (DB or transient memory)
        2. Load conversational history
        3. Save new user message (if auth)
        4. Analyze conversation state and extract slot preferences
        5. Execute state-specific actions:
           - RECOMMENDATION: Run product scoring, save recommendation trace (if auth), generate explanations.
        ...
        6. Persist assistant reply (if auth)
        7. Return Response
        """
        ...
        # --- Session Setup ---
        if is_guest:
            # Guest sessions are not stored on the backend.
            # Use client-provided session_id or generate a temporary one
            if not session_id or session_id.strip() == "":
                session_id = f"guest_{uuid.uuid4()}"
                logger.info(f"Generated new stateless guest session_id: {session_id}")
            
            # Map history from request Pydantic schema to dictionaries
            history = []
            if request.history:
        ...
        active_state, current_slots, purchase_intent, lead_info = StateService.analyze_message_and_update_state(
        ...
        if active_state == "RECOMMENDATION":
            ...
            # Log recommendation to recommendation_history table (only stores for auth users)
            ...
            if not is_guest:
                # Authenticated user: Capture lead automatically using profile metadata
                ...
                if not name or not phone:
                    if not name and not phone:
                        ...
                        assistant_reply = f"Thank you {name}! Could you please provide your phone number so our sales team can contact you to finalize the order?"
                    else:
                        assistant_reply = "I can help you place an order for that! Could you please share your name so our team knows who to contact?"
                else:
        ...
        # --- Persist Assistant Reply (Authenticated Users Only) ---
        if not is_guest:
            try:
